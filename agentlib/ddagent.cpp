    ...
    enter_critical_section(jvmti_env);
    {
        ...
        if (err == JVMTI_ERROR_NONE && thr_count >= 1)
        {
            ...
            for ( i=0; i < thr_count; i++)
            {
                ...
                jvmti->GetOwnedMonitorInfo(thr_ptr[i], &monitorCount, &ownedMonitors);
                for(jint j=0; j < monitorCount; ++j)
                {
                    jlong tag = -1;
                    jvmti->GetTag(ownedMonitors[j],&tag);
                    if(tag==0)
                    {
                        tag = gdata->tagCounter;
                        gdata->tagCounter = gdata->tagCounter+1;
                        jvmti->SetTag(ownedMonitors[j],tag);
                    }
                    printf("%d %x\n",tag, ownedMonitors[j]);
                }
                jvmti->Deallocate((unsigned char*)ownedMonitors);
                printf("-----------\n\n");

                jobject contendedMonitor;
                jvmti->GetCurrentContendedMonitor(thr_ptr[i], &contendedMonitor);
                if(contendedMonitor != NULL)
                {
                    jlong tag = -1;
                    jvmti->GetTag(contendedMonitor,&tag);
                    if(tag==0)
                    {
                        tag = gdata->tagCounter;
                        gdata->tagCounter = gdata->tagCounter+1;
                        jvmti->SetTag(contendedMonitor,tag);
                    }
                    printf("Contended Monitor: %d %x\n\n",tag, contendedMonitor);
                }
...
void Checkit( jthread* threadArray, jint thread_count)
{
    std::map<jint,jthread> monitorToThreadMap;
    std::list<jthread> threads;
    graph<jthread> threadGraph;
    for(jint i=0; i < thread_count; ++i)
    {
        threads.push_back( (threadArray[i]) );
    }

    //build monitorToThreadMap
    foreach(jthread thread, threads)
    {
        jint count=0;
        ...
        {
            jint hashCode;
            jvmti->GetObjectHashCode(monitors[i],&hashCode);
            monitorToThreadMap[hashCode]=thread;

        }
        //also, insert the thread into the graph
        threadGraph.insertVertex(thread);
        //free the monitors, we are done with them
        if(count>0)
        {
            jvmti->Deallocate((unsigned char*)monitors);
        }
    }
    ...
    foreach(jthread thread, threads)
    {
        ...
        jvmti->GetObjectHashCode(monitor,&hashCode);
        //jvmti->Deallocate((unsigned char*)monitor);
        jthread owner = monitorToThreadMap[hashCode];
        if(owner!=NULL)
        {
            //threadGraph.insertEdge(owner,thread);
            threadGraph.insertEdge(thread,owner);
        }
    }
    if(!threadGraph.noCycle(threadGraph))
    {
        std::list<jthread> outList;
        threadGraph.topologicalSort(threadGraph, outList);
        printf("I have a threadlist to pass to the gui!");
        //exit(0);
    }
